/**
 * News Categories Management Standalone Module
 * This file handles CRUD operations for News Categories independently from other scripts
 */

// Immediately invoke function to avoid global namespace pollution
(function() {
    // Debug information
    console.log('News Categories Standalone JS loaded!');
    console.log('Page URL:', window.location.pathname);

    // Prevent execution if not on the news categories page
    ...
    document.addEventListener('DOMContentLoaded', function() {
        console.log('DOM fully loaded - initializing News Categories JS');
        ...
        // Ensure any modals are hidden on page load
    ...
    function initializeEventListeners() {
        ...
        // Name and slug input for auto-generation
            ...
            <td class="py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                <div class="flex justify-end space-x-2">
                    ...
                    <button class="action-btn delete-btn" data-id="${item.id}" data-name="${item.name || 'Unnamed Category'}" aria-label="Delete" ${item.news_count > 0 ? 'disabled title="Cannot delete category with articles"' : ''}>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" width="18" height="18" ${item.news_count > 0 ? 'class="text-gray-400"' : ''}>
